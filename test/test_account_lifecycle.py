...
def test_delete_user_data_removes_account_sessions_memory_and_conversations(tmp_path, monkeypatch):
    ...
    conversation_file.write_text(
        json.dumps({"users": {email: {"conversations": {"abc": {"messages": []}}}}}),
        encoding="utf-8",
    )
    ...
    assert "student@example.com" not in json.loads(users_file.read_text(encoding="utf-8"))["users"]
    assert not user_dir.exists()
    assert "student@example.com" not in json.loads(conversation_file.read_text(encoding="utf-8"))["users"]
