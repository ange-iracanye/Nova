...
export default function NovaExperience() {
  ...
  const run = path => { setPaletteOpen(false); setDockOpen(false); navigate(path); };
  ...
  return <>
    ...
    {dashboard && <div className="nova-dashboard-hud" aria-hidden="true">
      <div className="nova-hud-dot" /> <span>LIVE LEARNING OS</span><span className="nova-hud-divider" /><span>ADAPTIVE</span>
    </div>}
    ...
    {paletteOpen && <div className="nova-command-backdrop" onMouseDown={() => setPaletteOpen(false)}>
      <div className="nova-command-panel" onMouseDown={event => event.stopPropagation()}>
        <div className="nova-command-header"><div className="nova-command-title"><Command size={17} /> Nova command center</div><button onClick={() => setPaletteOpen(false)}><X size={17} /></button></div>
        ...
        <div className="nova-command-list">
          {filtered.map((item, index) => { const Icon = item.icon; return <button key={item.path} onClick={() => run(item.path)} className="nova-command-item"><span className="nova-command-icon"><Icon size={17} /></span><span><strong>{item.label}</strong><small>{item.hint}</small></span><span className="nova-command-index">{index + 1}</span><ArrowRight size={15} /></button>; })}
