...
const FEATURES = [
    [Brain, "Adaptive tutoring", "Explanations and difficulty evolve around your actual understanding.", "adaptive-tutoring"],
    [Layers3, "Learning memory", "Nova keeps useful learning context across sessions and can use relevant history.", "learning-memory"],
    [LineChart, "Progress intelligence", "See mastery, weak areas, momentum and what to study next.", "progress-intelligence"],
    [FileText, "Learn from material", "Turn notes and documents into focused learning sessions.", "learn-from-material"],
    [Target, "Goal-driven study", "Move from vague intentions to concrete study targets.", "goal-driven-study"],
    [Bot, "Built to teach", "Nova is designed around understanding, practice and retention.", "built-to-teach"],
];
...
function FeatureCard({ icon: Icon, title, text, index, slug }) {
    return (
        <Link to={`/capabilities/${slug}`} data-nova-reveal className="nova-reveal group relative block overflow-hidden rounded-3xl border border-white/[0.07] bg-white/[0.025] p-6 transition duration-500 hover:-translate-y-2 hover:border-cyan-300/20 hover:bg-white/[0.05]" style={{ transitionDelay: `${index * 70}ms` }}>
...
export default function Home() {
    ...
    return (
        <main className="min-h-screen overflow-hidden bg-[#020617] text-white selection:bg-cyan-300/20">
            ...
            <header className="relative z-20 mx-auto flex max-w-7xl items-center justify-between px-6 py-6 lg:px-8"><Link to="/" className="flex items-center gap-3"><NovaLogo className="h-10 w-10 rounded-xl object-contain"/><div><div className="text-sm font-bold tracking-[.18em]">NOVA</div><div className="text-[10px] tracking-[.2em] text-slate-500">AI LEARNING</div></div></Link><nav className="hidden items-center gap-1 md:flex"><Link to="/chat" className="rounded-xl px-4 py-2 text-sm text-slate-400 transition hover:bg-white/[.05] hover:text-white">Learn</Link><Link to="/dashboard" className="rounded-xl px-4 py-2 text-sm text-slate-400 transition hover:bg-white/[.05] hover:text-white">Dashboard</Link><Link to="/settings" className="rounded-xl px-4 py-2 text-sm text-slate-400 transition hover:bg-white/[.05] hover:text-white">Settings</Link><Link to="/about" className="rounded-xl px-4 py-2 text-sm text-slate-400 transition hover:bg-white/[.05] hover:text-white">About</Link></nav><Link to="/chat" className="group hidden items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-cyan-600 transition hover:-translate-y-0.5 hover:bg-slate-100 sm:flex">Start learning <ArrowRight size={15} className="transition group-hover:translate-x-1"/></Link></header>
            <section className="relative z-10 mx-auto grid min-h-[calc(100vh-88px)] max-w-7xl items-center gap-12 px-6 pb-28 pt-10 lg:grid-cols-[1.05fr_.95fr] lg:px-8"><div><div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/10 bg-cyan-300/[.06] px-3.5 py-2 text-xs font-medium text-cyan-200 backdrop-blur-xl"><span className="relative flex h-2 w-2"><span className="absolute h-full w-full animate-ping rounded-full bg-cyan-300 opacity-60"/><span className="relative h-2 w-2 rounded-full bg-cyan-300"/></span>Adaptive intelligence for learning</div><h1 className="mt-7 max-w-4xl text-5xl font-semibold leading-[.94] tracking-[-.055em] sm:text-7xl lg:text-[5.9rem]">Your learning.<span className="block bg-gradient-to-r from-cyan-200 via-white to-violet-300 bg-clip-text text-transparent">Elevated.</span></h1><p className="mt-7 max-w-2xl text-base leading-7 text-slate-400 sm:text-lg">Nova is an adaptive AI tutor that explains, remembers, challenges and guides you toward real understanding.</p><div className="mt-9 flex flex-col gap-3 sm:flex-row"><Link to="/chat" className="group inline-flex items-center justify-center gap-2.5 rounded-2xl bg-white px-6 py-4 text-sm font-bold text-cyan-600 shadow-[0_20px_70px_rgba(255,255,255,.08)] transition duration-300 hover:-translate-y-1"><Play size={16} fill="currentColor"/>Enter Nova<ArrowRight size={16} className="transition group-hover:translate-x-1"/></Link><Link to="/dashboard" className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[.035] px-6 py-4 text-sm font-medium backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:border-white/20">Explore your progress <LineChart size={16}/></Link></div><div className="mt-10 flex flex-wrap gap-x-6 gap-y-3 text-xs text-slate-500">{["Adaptive explanations","Learning memory","Progress intelligence"].map(item=><span key={item} className="flex items-center gap-2"><CheckCircle2 size={14} className="text-emerald-400"/>{item}</span>)}</div></div><div className="relative flex min-h-[560px] items-center justify-center" style={orbStyle}><div className="nova-pulse absolute h-80 w-80 rounded-full bg-cyan-400/[.09] blur-[90px]"/><div className="nova-spin absolute h-[480px] w-[480px] rounded-full border border-white/[.06]"/><div className="nova-spin absolute h-[350px] w-[350px] rounded-full border border-dashed border-cyan-300/[.12]" style={{animationDirection:"reverse",animationDuration:"17s"}}/><div className="nova-float relative flex h-64 w-64 items-center justify-center rounded-[4.5rem] border border-white/10 bg-gradient-to-br from-cyan-300/20 via-slate-950 to-violet-500/20 shadow-[0_0_120px_rgba(34,211,238,.12)] backdrop-blur-2xl"><div className="absolute inset-5 rounded-[3.5rem] border border-white/[.07] bg-black/30"/><div className="relative flex h-28 w-28 items-center justify-center rounded-[2.2rem] border border-cyan-200/20 bg-white/[.055] shadow-[0_0_60px_rgba(34,211,238,.13)]"><Sparkles size={42}/></div><div className="absolute -right-10 top-10 rounded-2xl border border-white/10 bg-slate-950/80 p-4 shadow-2xl backdrop-blur-xl"><div className="flex items-center gap-3"><Zap size={17} className="text-cyan-300"/><div><div className="text-xs font-semibold">Adaptive</div><div className="text-[10px] text-slate-500">Teaching mode</div></div></div></div><div className="absolute -left-12 bottom-12 rounded-2xl border border-white/10 bg-slate-950/80 p-4 shadow-2xl backdrop-blur-xl"><div className="flex items-center gap-3"><Trophy size={17} className="text-violet-300"/><div><div className="text-xs font-semibold">Progress</div><div className="text-[10px] text-slate-500">Always moving</div></div></div></div></div><div className="absolute bottom-5 left-1/2 w-72 -translate-x-1/2 rounded-2xl border border-white/10 bg-slate-950/70 p-3 text-center text-xs text-slate-400 backdrop-blur-xl"><span className="text-white">{STEPS[step][1]}</span> · {STEPS[step][2]}</div></div></section>
