...
export default function DemoHome() {
    ...
    return <main className="min-h-screen overflow-hidden bg-[#050816] text-white selection:bg-cyan-300/20">
        ...
        {menu && <div className="relative z-30 mx-4 rounded-2xl border border-white/10 bg-slate-950/90 p-3 backdrop-blur-xl md:hidden"><Link onClick={()=>setMenu(false)} to="/chat" className="block rounded-xl px-4 py-3 text-sm hover:bg-white/5">Try Nova</Link><Link onClick={()=>setMenu(false)} to="/login" className="block rounded-xl px-4 py-3 text-sm hover:bg-white/5">Sign in</Link><Link onClick={()=>setMenu(false)} to="/register" className="block rounded-xl bg-white px-4 py-3 text-sm font-semibold text-slate-950">Create account</Link></div>}
            ...
            <div className="relative flex min-h-[430px] items-center justify-center lg:min-h-[600px]" style={orb}>
                ...
                <div className="nova-float relative w-[min(78vw,330px)] rounded-[2.8rem] border border-white/10 bg-gradient-to-br from-cyan-300/15 via-slate-950 to-violet-500/15 p-5 shadow-[0_0_100px_rgba(34,211,238,.12)] backdrop-blur-2xl">
                    <div className="rounded-[2.2rem] border border-white/[.07] bg-black/25 p-5">
                        ...
                        <Link to="/chat" className="mt-4 flex items-center justify-center gap-2 rounded-xl bg-white py-3 text-xs font-bold text-slate-950">Open demo <ArrowRight size={14}/></Link>
                    </div>
                </div>
            </div>
        </section>
