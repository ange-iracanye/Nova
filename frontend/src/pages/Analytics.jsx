...
function Stat({ icon, label, value, detail, accent = "sky" }) { return <Card className="analytics-card"><div className={`flex h-10 w-10 items-center justify-center rounded-xl bg-${accent}-400/10 text-${accent}-300`}>{icon}</div><div className="mt-5 text-3xl font-bold tracking-tight tabular-nums">{value}</div><div className="mt-1 text-sm text-slate-400">{label}</div>{detail && <div className="mt-2 text-xs text-slate-600">{detail}</div>}</Card>; }
