    ...
    try:
        ...
        if method == "GET" and path in {"/auth/session", "/auth/me"}:
            ...
            if not session:
                await _send_json(
                    send,
                    401 if path == "/auth/me" else 200,
                    {"success": path == "/auth/session", "authenticated": False, "user": None, "session": None},
                    _cors_headers(scope),
                )
                return True
            await _send_json(
                send,
                200,
                {"success": True, "authenticated": True, "user": {"email": session["email"]}, "session": session},
                _cors_headers(scope),
            )
            return True
        ...
        if method == "POST" and path == "/auth/logout":
            ...
            await _send_json(
                send,
                200,
                {"success": True, "logged_out": True, "session_removed": bool(session)},
                _cors_headers(scope),
        ...
        if method != "POST" or path not in {"/register", "/login"}:
            await _send_json(send, 405, {"success": False, "error": {"code": "METHOD_NOT_ALLOWED", "message": "Method not allowed."}}, _cors_headers(scope))
            return True
        ...
        try:
            ...
            await _send_json(send, 400, {"success": False, "error": {"code": "INVALID_JSON", "message": "Please provide valid JSON."}}, _cors_headers(scope))
            return True
        ...
        if path == "/register":
            try:
                ...
                traceback.print_exc()
                await _send_json(send, 500, {"success": False, "error": {"code": "REGISTRATION_ERROR", "message": "Registration failed."}}, _cors_headers(scope))
                return True
            if not created:
                await _send_json(send, 409, {"success": False, "message": "An account with this email may already exist.", "email": email}, _cors_headers(scope))
                return True
        else:
            try:
                ...
                traceback.print_exc()
                await _send_json(send, 500, {"success": False, "error": {"code": "LOGIN_ERROR", "message": "Login failed."}}, _cors_headers(scope))
                return True
            if not valid:
                await _send_json(send, 401, {"success": False, "message": "Incorrect email or password.", "email": None}, _cors_headers(scope))
                return True
        ...
        await _send_json(
            send,
            200,
            {"success": True, "email": email, "user": {"email": email}, "session": session},
            _cors_headers(scope),
            [_cookie_header(session["token"])],
        )
        return True
    except Exception:
