...
if ENABLE_DOCS: PUBLIC_PATHS.update({"/docs", "/redoc", "/openapi.json"})
if ENABLE_DEMO: PUBLIC_PATHS.add("/demo/session")
...
if IS_PRODUCTION and COOKIE_SAMESITE == "none": COOKIE_SAMESITE = "lax"
...
def _cookie_session_token(request: Request) -> str | None:
    ...
    try: expires_at = datetime.fromisoformat(str(session.get("expires_at")))
    except (TypeError, ValueError): return None
    if expires_at <= datetime.now(timezone.utc):
        api.auth_sessions.pop(token, None); return None
    return token
...
async def _set_login_cookie(response: Response) -> Response:
    ...
    if not isinstance(body, (bytes, bytearray)): return response
    try: payload = json.loads(bytes(body).decode("utf-8")); token = payload.get("session", {}).get("token")
...
async def production_auth_boundary(request: Request, call_next):
    ...
    if not IS_PRODUCTION: return await call_next(request)
    content_length = request.headers.get("content-length")
    if content_length:
        try: size = int(content_length)
        except ValueError: return JSONResponse(status_code=400, content={"success": False, "error": {"code": "INVALID_CONTENT_LENGTH", "message": "Invalid request size."}})
        limit = MAX_UPLOAD_BYTES if request.url.path.startswith("/upload") else MAX_BODY_BYTES
        if size > limit: return JSONResponse(status_code=413, content={"success": False, "error": {"code": "REQUEST_TOO_LARGE", "message": "Request is too large."}})
    if _rate_limited(request): return JSONResponse(status_code=429, content={"success": False, "error": {"code": "RATE_LIMITED", "message": "Too many requests. Please try again shortly."}})
    path = request.url.path
    if path in PUBLIC_PATHS or any(path.startswith(prefix) for prefix in PUBLIC_PREFIXES):
        response = await call_next(request)
        if path in {"/login", "/register"}: response = await _set_login_cookie(response)
        if path == "/auth/logout": response.delete_cookie(COOKIE_NAME, path="/")
        return response
    cookie_token = _inject_cookie_session(request)
    if not cookie_token and not api.get_auth_session(request): return JSONResponse(status_code=401, content={"success": False, "error": {"code": "NOT_AUTHENTICATED", "message": "A valid Nova session is required."}})
    response = await call_next(request)
    return response
...
async def production_security_headers(request: Request, call_next):
    ...
    if request.url.path.startswith("/auth/"): response.headers.setdefault("Cache-Control", "no-store")
    if IS_PRODUCTION: response.headers.setdefault("Strict-Transport-Security", "max-age=31536000; includeSubDomains")
    return response
