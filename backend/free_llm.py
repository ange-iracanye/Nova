...
class FreeLLM:
    ...
    FRESHNESS_TERMS = {"today", "now", "currently", "current", "latest", "recent", "recently", "right", "happening", "news", "breaking", "this week", "this month", "yesterday"}
    ...
    def __init__(self, model: Optional[str] = None, max_retries: int = 1, retry_delay: float = 1.0):
        ...
        self.max_retries = max(0, int(max_retries)); self.retry_delay = max(0.0, float(retry_delay))
        self.last_error = None; self.last_response = None; self.last_generation_time = None
    ...
    def _fetch_web_context(cls, user: str) -> str:
        """Fetch a small, best-effort live search context for freshness-sensitive questions."""
        if not cls._needs_fresh_web_context(user): return ""
        try:
            from ddgs import DDGS
            with DDGS() as ddgs:
                results = list(ddgs.text(str(user).strip(), max_results=5))
    ...
    def answer(self, system: str, user: str, creativity: str = "medium") -> str:
        ...
        settings = self.get_generation_settings(creativity); self.total_requests += 1; started = time.perf_counter()
        original_user = user
        ...
        for model in models:
            for attempt in range(self.max_retries + 1):
                try:
                    ...
                    self.last_response = text; self.last_generation_time = time.perf_counter() - started; self.last_error = None; self.successful_requests += 1
                    return text
